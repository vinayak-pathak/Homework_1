...
```
The tail on the right after the winsorisation moves towards the center, and this removes the out 
...
In the first case before log transformation the data is positively skewed.But it fails to achieve the normalisation around a mean centered value after the log transformation and is still skewed.  The bin at 0 shifts the density curve towards higher side on the left, which means there are lot of data close to zero in the oldpeak and log transformation after adding 1 makes it zero, (since log1=0). So the given dataset cannot be transformed into mean centered data using the log transformation. For having the mean centered the data should look like followinng as in case of the cholestrol level(not asked in the question, just giving for an example).
```{r}
xinitoldpeak<-heart_df$oldpeak
dfoldpeak<-data.frame(xinitoldpeak)
dfoldpeak["xinitoldpeaklog"]<-data.frame(log(xinitoldpeak+1))
```
```{r}
plot7<-ggplot(dfoldpeak, aes(x=xinitoldpeak))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=1, fill="white", color="brown")+labs(x="ST depression induced by exercise relative to rest", y="Density")+geom_density(fill="#9900CC",  alpha=0.1)+ggtitle("Histogram before the log transformation")+theme(plot.title = element_text(hjust=0.5))

plot8<-ggplot(dfoldpeak, aes(x=xinitoldpeaklog))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=0.1, fill="white", color="brown")+labs(x="ST depression induced by exercise relative to rest", y="Density")+geom_density(fill="#9900CC",  alpha=0.05)+ggtitle("Histogram after the log transformation")+theme(plot.title = element_text(hjust=0.5))
plot(plot5/plot6)
