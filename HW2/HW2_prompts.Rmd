...
```
```{r}
p1<-ggplot(covid_df_USA, aes(x=date, y=new_cases_per_million))+geom_point(shape=3)+geom_line()
```
```{r}
p2<-ggplot(covid_df_USA, aes(x=date, y=new_deaths_per_million))+geom_point(shape=3)+geom_line()
```
```{r}
p3<-ggplot(covid_df_USA, aes(x=date, y=positive_rate))+geom_point(shape=3)+geom_line()
```
```{r}
plot(p1)
```
...
```{r}
t1 <- ggplot(heart_df, aes(x=trestbps))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=10, fill="white", color="brown")+geom_density()+labs(x="Resting Blood Pressure(mm/Hg)", y="Density")
```
```{r}
t2 <- ggplot(heart_df, aes(x=chol))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=10, fill="white", color="brown")+geom_density()+labs(x="Cholestrol", y="Density")
```
```{r}
t3 <- ggplot(heart_df, aes(x=thalach))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=10, fill="white", color="brown")+geom_density()+labs(x="Maximum heart Rate", y="Density")
```
```{r}
t4 <- ggplot(heart_df, aes(x=oldpeak))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=0.5, fill="white", color="brown")+geom_density()+labs(x="ST depression induced by exercise relative to rest", y="Density")
```
The above choice of binwidth allows the density curves to overlap appropriately over the histogram and show the the approprialte distribuition
```{r}
#add legends
plot((t1|t2)/(t3|t4))
```
...
```{r}
mu1<-ddply(heart_df, "sex", summarise, s)
```
