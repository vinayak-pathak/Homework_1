...
```
```{r}
p1<-ggplot(covid_df_USA, aes(x=date, y=new_cases_per_million))+geom_point(shape=1, size=0.1, alpha=0.5)+geom_line(color ="red")+theme(axis.title.y = element_blank())+ggtitle("New cases per million")+theme(plot.title=element_text(hjust = 0.5))+scale_x_date(date_breaks = "1 month", date_labels = "%b")+xlab("Date(Month)")
```
```{r}
p2<-ggplot(covid_df_USA, aes(x=date, y=new_deaths_per_million))+geom_point(shape=1, size=0.1, alpha =0.5)+geom_line(color ="red")+theme(axis.title.y = element_blank())+ggtitle("New deaths per million")+theme(plot.title=element_text(hjust = 0.5))+scale_x_date(date_breaks = "1 month", date_labels = "%b")+xlab("Date(Month)")
``` 
```{r}
p3<-ggplot(covid_df_USA, aes(x=date, y=positive_rate))+geom_point(shape=1,size=0.1, alpha =0.5)+geom_line(color ="red")+theme(axis.title.y = element_blank())+ggtitle("Positive Rate")+theme(plot.title=element_text(hjust = 0.5))+scale_x_date(date_breaks = "1 month", date_labels = "%b")+xlab("Date(Month)")
```
...
```
```{r}
t2 <- ggplot(heart_df, aes(x=chol))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=10, fill="white", color="brown")+geom_density()+labs(x="Cholestrol", y="Density")+ theme(legend.position="right")
```
```{r}
...
```
```{r}
t4 <- ggplot(heart_df, aes(x=oldpeak))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=0.2, fill="white", color="brown")+geom_density()+labs(x="ST depression induced by exercise relative to rest", y="Density")+ theme(legend.position="right")
```
...
t21<-ggplot(heart_df, aes(x=chol, color=diagnosed)) +
  geom_density()+geom_vline(data=mean2, aes(xintercept=grp.mean, color=diagnosed),linetype="dashed")+labs(x="Cholestrol", y="Density")
...
Cholestrol --- Positively Skewed, Highly Skewed
Maximum Blood Pressure ---Negatively Skewed, Moderately Skewed
ST depressionn induced by exercise relative to rest---Positively Skewed, Highly Skewed
...
```{r}
if(class(heart_data$sex)!='integer')
...
xinit=heart_df$chol
xmod<-winsor(xinit, 0.05)

```

```{r}
t3 <- ggplot(heart_df, aes(x=xinit))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=20, fill="white", color="black")+geom_density(fill="#CC0099",  alpha=0.2)+labs(x="Cholestrol", y="Density")+xlim(0, 600)+ggtitle("Before Winsorisation")+theme(plot.title = element_text(hjust = 0.5))
dfchol <-data.frame(xmod)
t7 <- ggplot(dfchol, aes(x=xmod))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=20, fill="white", color="black")+geom_density(fill="#9900CC",  alpha=0.3)+labs(x="Cholestrol", y="Density")+xlim(0, 600)+ggtitle("After Winsorisation")+theme(plot.title = element_text(hjust = 0.5))
plot(t3/t7)
...
```{r}
dfchol["xinitlog"]<-data.frame(log(xinit+1))
```
```{r}
plot5<-ggplot(dfchol, aes(x=xinit))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=20, fill="white", color="brown")+labs(x="Cholestrol", y="Density")+geom_density(fill="#9900CC",  alpha=0.1)+ggtitle("Histogram before the log transformation")+theme(plot.title = element_text(hjust=0.5))

plot6<-ggplot(dfchol, aes(x=xinitlog))+geom_histogram(aes(y=..density..), position="identity", alpha=0.5, binwidth=0.1, fill="white", color="brown")+labs(x="Cholestrol", y="Density")+geom_density(fill="#9900CC",  alpha=0.05)+ggtitle("Histogram after the log transformation")+theme(plot.title = element_text(hjust=0.5))
plot(plot5/plot6)

```
In the first case before logg transformation the data is positively skewed, whereas in the seconnd case the data almost centered arounnd a mean value, and not skewed.